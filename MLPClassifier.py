""" MLPClassifier - A classification multi layer perceptron
Author: Wesley Silva
Github: wesleyit
2017
"""
...
class MLPClassifier:
    ...
    for classification tasks. It is bundled with 1 input layer, 1 hidden
    layer and 1 output layer. You can choose the neurons in each layer
    and set the hyperparameters.

    It is a classification neural net because the output layer uses a
    sigmoid function, which can return an output from 0.0 to 1.0.
    This value may be rouded, as well. This behavior is different from
    the regression neural network, which features a linear output able
    to return any numerical value.
    """
    ...
    def __init__(self, n_inputs, n_hidden, n_outputs, rate, epochs,
        ...
        Returns:
            A MPPClassifier object ready to be trained.
        """
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.epochs):
            ...
            # The vars error and yhat have the same shape.
            # grad_w2 must have the same shape, so we cannot
            # use the dot product here, we do a simple
            # multiplication instead.
            # The shape is [N, O], where N is the number of rows for this sample
            # and O is the number of output neurons.
            grad_w2 = error * yhat * (1 - yhat)
